...
def monthly_page():
    ...
    if tab == 'summary':
        data = db.fetch_monthly_summary(from_date, to_date, offset=offset, limit=per_page, search=search)
        total = db.count_monthly_summary(from_date, to_date, search=search)
        return render_template(
            "pages/monthly_summary.html",
            data=data,
            current_page=page,
            total_pages=(total + per_page - 1) // per_page,
            active_tab='summary',
